...
class WatermarkBase:
    def __init__(
        ...
        self.delta = delta
        self.rng = None
        self._initialize_seeding_scheme(seeding_scheme, new_hash_key)
        # Legacy behavior:
        self.select_green_tokens = select_green_tokens

    def _initialize_seeding_scheme(self, seeding_scheme: str, new_hash_key) -> None:
        """Initialize all internal settings of the seeding strategy from a colloquial, "public" name for the scheme."""
        self.prf_type, self.context_width, self.self_salt, self.hash_key = seeding_scheme_lookup(seeding_scheme, new_hash_key)
...
class WatermarkLogitsProcessor(WatermarkBase, LogitsProcessor):
    ...
    def get_greenlist_ids(self, input_ids=None):
        self.rng = torch.Generator(device="cuda") if self.rng is None else self.rng
        if input_ids is not None:
            return self._get_greenlist_ids(torch.tensor([input_ids]).to("cuda"))
        else:
            return self._get_greenlist_ids(torch.tensor([0.]).to("cuda"))
