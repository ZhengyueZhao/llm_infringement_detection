...
def seeding_scheme_lookup(seeding_scheme: str, new_hash_key=None):
...
def static_prf(input_ids: torch.LongTensor, salt_key: int) -> int:
    return hashint(torch.tensor([salt_key])).item()
